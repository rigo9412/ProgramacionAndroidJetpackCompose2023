...
fun calcularNumeroLetras(cad: String):String {
    var cadd = cad.lowercase().unaccent().replace(("[^\\w]").toRegex(), "")
    var map :MutableMap<String, Int> = mutableMapOf()
    var cont = 0
    var tamanio = cadd.length
    
    while(cont < tamanio) {
        var lett = cadd[cont].toString()
