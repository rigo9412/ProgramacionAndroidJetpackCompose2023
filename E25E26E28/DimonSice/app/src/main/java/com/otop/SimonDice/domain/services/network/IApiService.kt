package com.otop.SimonDice.domain.services.network

import com.otop.SimonDice.domain.models.get.GetAllTopResponse
import com.otop.SimonDice.domain.models.get.GetTopResponse
import com.otop.SimonDice.domain.models.post.PostRequestTop
import retrofit2.http.Body
import retrofit2.http.GET
import retrofit2.http.Headers
import retrofit2.http.POST

interface IApiService {
    companion object {
        const val BASE_URL = "https://simon-game-api-production.up.railway.app/"
        const val ACCESS_TOKEN = ""
    }

    @Headers("Authorization: Bearer $ACCESS_TOKEN")
    @GET("api/tops")
    suspend fun getTop(): GetAllTopResponse

    @Headers("Authorization: Bearer $ACCESS_TOKEN")
    @POST("api/tops")
    suspend fun postTop(@Body requestTop: PostRequestTop): GetTopResponse
}