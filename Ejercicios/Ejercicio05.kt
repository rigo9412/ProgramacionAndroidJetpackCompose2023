...
Notas: la letra con sus variantes es la misma letra:
a = A
a = ā
a = á
...
fun contarletras(str: String): String{
    var texto = str
    texto = texto.lowercase()
    texto = Normalizer.normalize(texto, Normalizer.Form.NFD)
    var map = HashMap<Char,Int>()
    for (i in 0..texto.lastIndex){
        if(texto[i].isLetter())
        {
            map.merge(texto[i],1)//llave, valor
            {
                llave, value -> llave + value  //si existe, le sumamos el nuevo valor al anterior
            }
        }
    }
    var mapfinal = map.toString().replace(" ", "")
    return mapfinal.substring(1,mapfinal.length-1)
}