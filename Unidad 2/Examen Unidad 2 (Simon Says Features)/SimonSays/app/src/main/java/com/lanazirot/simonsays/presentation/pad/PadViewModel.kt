...
class PadViewModel @Inject constructor(private val gameManager: IGameManager) : ViewModel() {
    ...
    private fun gameOver() {
        ...
        if(gameManager.scoreIsGoingToBeInTheTopTen(Score(score = _pad.value.player?.score ?: 0, name = ""))) {
            _pad.value = PadState(gameStatus = GameStatus.GAME_OVER, isGoingToScoreboard = true, player = _pad.value.player)
        }else{
    ...
    fun compareStep(inputStep: StepColorAction) {
        ...
        if (!isStepCorrect) {
            _pad.value = PadState(player = _pad.value.player?.score?.let { _pad.value.player?.copy(score = it) })
            this.gameOver()
