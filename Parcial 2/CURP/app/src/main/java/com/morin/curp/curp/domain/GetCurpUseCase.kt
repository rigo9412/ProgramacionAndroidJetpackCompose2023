...
import com.morin.curp.curp.ui.form.CurpFormModelState
import com.morin.curp.curp.data.*
...
class GetCurpUseCase {
    ...
    fun invoque(data: CurpFormModelState): ResultCase {
        try{
            ...
            var lastname = if(inputData.lastName.isEmpty() || inputData.lastName.isBlank()){
                "X"
            }else {
                clean(inputData.lastName)
            }
            curp += middleName[0]
