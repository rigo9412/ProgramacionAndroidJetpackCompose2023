...
@Composable
fun MainScreen(viewModel: MainViewModel, topVM: TopViewModel) {
    val currentScreen: Screen by viewModel.currentScreen.collectAsState()

    when (currentScreen) {
        is Screen.Game -> SimonGame(viewModel,topVM = topVM)
        is Screen.Top -> TopScreen(viewModel,topVM = topVM)
        else -> {}
    }
}

@Composable
fun SimonGame(viewModel: MainViewModel, topVM: TopViewModel) {
    Column(
        ...
        Column() {
            if (viewModel.gameOver.value) {
                ...
                Button(onClick = { val newPlayer = Player(1,viewModel.topName.value,viewModel.level.value,53); topVM.postTopFake(newPlayer); viewModel.gameOver.value = false}) {
...
@Composable
fun TopScreen(viewModel: MainViewModel, topVM: TopViewModel): Unit {
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text(text = "Top 10", fontSize = 40.sp)
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center, ) {
            LazyColumn(modifier = Modifier.fillMaxHeight()) {
                items(items = topVM.dataPlayer.value.take(10)){player ->
